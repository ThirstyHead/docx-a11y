"""Generate multi-format icons (.png, .ico, .icns) for docx-a11y."""
import os
import shutil
import subprocess
...
def render_master_image(size: int = 512) -> Image.Image:
    ...
    # Document sheet
    doc_x0, doc_y0 = int(size * 0.22), int(size * 0.16)
    doc_x1, doc_y1 = int(size * 0.78), int(size * 0.84)
    draw.rounded_rectangle(
    ...
    # Blue DOCX badge (Word signature blue)
    badge_x0, badge_y0 = int(size * 0.18), int(size * 0.25)
    badge_x1, badge_y1 = int(size * 0.46), int(size * 0.35)
    draw.rounded_rectangle(
...
def main():
    ...
    # Build macOS .icns
    icns_path = icons_dir / "docx-a11y.icns"
    iconutil = shutil.which("iconutil")
    if iconutil:
        iconset_dir = icons_dir / "docx-a11y.iconset"
