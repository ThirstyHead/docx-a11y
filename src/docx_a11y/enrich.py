...
``get_criterion_text(sc)`` returns markdown text or ``None``.
"""
import json
import os
import re
...
from .audit import load_result  # noqa: F401  (kept for API symmetry)
...
def find_server() -> Optional[Path]:
    ...
    if shutil.which("wcag-guidelines-mcp"):
        return Path(shutil.which("wcag-guidelines-mcp"))
...
def fetch_via_server(server: Path, sc: str, timeout: float = 20.0) -> Optional[str]:
    ...
    try:
        init = {"jsonrpc": "2.0", "id": 1, "method": "initialize", "params": {
            ...
            "clientInfo": {"name": "docx-a11y", "version": "0.1.0"}}}
        ...
        # read initialize response, then send notification + call
